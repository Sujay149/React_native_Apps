...
TaskTrack is an Expo + React Native task tracking app with backend auth integration and clean global state management.

## Features

- Backend-connected login and signup (`/auth/login`, `/auth/signup`)
...
0. Set backend URL (optional but recommended for physical devices):

```bash
EXPO_PUBLIC_BACKEND_URL=http://<your-machine-ip>:8080
```

If not set, the app defaults to `http://10.0.2.2:8080` on Android emulator and `http://localhost:8080` elsewhere.
